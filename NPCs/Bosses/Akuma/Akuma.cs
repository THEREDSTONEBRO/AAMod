			...
			if (Main.netMode != 1)
			{
				if (!TailSpawned && npc.ai[0] == 0f)
	            {
	                ...
	                for (int num36 = 0; num36 < maxLength; num36++)
	                {
	                    ...
	                    {
	                        lol = NPC.NewNPC((int)npc.position.X + (npc.width / 2), (int)npc.position.Y + (npc.height / 2), mod.NPCType("AkumaBody"), npc.whoAmI);
	                    }
	                    else
	                    {
	                        lol = NPC.NewNPC((int)npc.position.X + (npc.width / 2), (int)npc.position.Y + (npc.height / 2), mod.NPCType("AkumaTail"), npc.whoAmI);
	                    }
	                    if (num36 % 2 == 0)
	                    {
	                    ...
	                    {
	                    	addOrbiter = 0;
	                    	Main.npc[lol].localAI[1] = 1f;
	                    }
	                    if (addOrbiter2 >= 3)
	                    {
	                    	addOrbiter2 = 0;
	                    	Main.npc[lol].localAI[2] = 1f;
	                    }
