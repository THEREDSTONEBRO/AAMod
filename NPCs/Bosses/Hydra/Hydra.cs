...
namespace AAMod.NPCs.Bosses.Hydra
{
    ...
    public class LimbInfo
    {
        ...
        public Hydra hydra = null;
    }

    public class LegInfo : LimbInfo
    {
        Vector2 velocity, oldVelocity, legOrigin;
        float velOffsetY = 0f, distanceToMove = 60f, distanceToMoveX = 25f;
        ...
        public LegInfo(int lType, Vector2 initialPos, Hydra m)
        {
            hydra = m;
            position = initialPos;
            pointToStandOn = position;
            limbType = lType;
            Hitbox = new Rectangle(0, 0, 38, 24);
        ...
        public Vector2 GetStandOnPoint(NPC npc)
        {
            ...
            float standOnX = npc.Center.X + hydra.topVisualOffset.X + (limbType == 3 ? (-outerLegDefault - Hitbox.Width) : limbType == 2 ? (outerLegDefault + Hitbox.Width) : limbType == 1 ? (-innerLegDefault - Hitbox.Width) : (innerLegDefault + Hitbox.Width));
            ...
            if (tileY - defaultTileY > Hydra.flyingTileCount) { return default(Vector2); } //'flying' behavior
            if (!flying && !hydra.flying)
            {
            ...
            return ((flying || hydra.flying) ? default(Vector2) : defaultPlacement);
        }

        public Vector2 GetMantidConnector(NPC npc)
        {
            return npc.Center + hydra.topVisualOffset + new Vector2((limbType == 3 || limbType == 1 ? -40f : 40f), 0f);
        }
        ...
        public void DrawLeg(SpriteBatch sb, NPC npc, Color dColor)
        {
            ...
            if (textures == null)
            {
                ...
                textures[0] = mod.GetTexture("NPCs/Bosses/Hydra/HydraLegCapL");
				textures[1] = mod.GetTexture("NPCs/Bosses/Hydra/HydraLegL");
                textures[2] = mod.GetTexture("NPCs/Bosses/Hydra/HydraLegCapR");
				textures[3] = mod.GetTexture("NPCs/Bosses/Hydra/HydraLegR");				
                textures[4] = mod.GetTexture("NPCs/Bosses/Hydra/HydraFoot");
            }
