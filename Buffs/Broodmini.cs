...
namespace AAMod.Buffs
{
	public class Broodmini : ModBuff
	{
		...
		public override void Update(Player player, ref int buffIndex)
		{
			...
			{
				Projectile.NewProjectile(player.position.X + (float)(player.width / 2), player.position.Y + (float)(player.height / 2), 0f, 0f, mod.ProjectileType("Broodmini"), 0, 0f, player.whoAmI, 0f, 0f);
			}
		}
	}
}