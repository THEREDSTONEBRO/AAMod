...
namespace AAMod.Base
{
	/*public class BaseInvasion
	{
		...
		public void OnWorldUpdate()
		{
			...
			undeadipedeCount = BaseMod.BaseAI.GetNPCs(default(Vector2), GRealm.UNDEADIPEDE_TYPES[0], -1, null).Length + BaseMod.BaseAI.GetNPCs(default(Vector2), GRealm.UNDEADIPEDE_TYPES[1], -1, null).Length;	
		...
		public void HandleHardMobSpawn(int type)
		{
            if (Main.netMode != 1 && IsHardMob(type))
			{
				...
				if(chat != null) BaseMod.BaseUtility.Chat(chat, BaseMod.BaseConstants.CHATCOLOR_PURPLE);				
			}	
		}
		...
		public static IDictionary<int, float> SpawnPool(IDictionary<int, float> startingPool, int x, int y, Player player)
		{
			...
			}else //normal zombies
			{
				...
				if (ZONEDEV || BaseMod.BaseTile.LiquidCount(x, y, 25, 0) > 120) //floaty zombie
				{
